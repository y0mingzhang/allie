...
class ChessLLM:
    ...
    def get_best_move(self, board, num_tokens=None, conversation=None):
        ...
        assert (
            num_tokens >= 9
        ), "A single move might take as many as 9 tokens (3 for the number + 6 for, e.g., 'N3xg5+)."
            ...
            if conversation:
                if board.ply() > 0:
                    conversation.send_message(
                        "player",
                        f"You played a move already in my cache (because I predicted it or someone already played it)! Returning {out}.",
                    )
                    conversation.send_message(
                        "spectator",
                        f"Player played a move already in my cache (because I predicted it or someone already played it). Returning {out}.",
                    )
            return out
        ...
        if conversation:
            conversation.send_message(
                "player",
                f"Querying {self.config['model']} with ... {pgn_to_query.split(']')[-1][-90:]}",
            )
            conversation.send_message(
                "spectator",
                f"Querying {self.config['model']} with ... {pgn_to_query.split(']')[-1][-90:]}",
        ...
        if conversation:
            conversation.send_message(
                "player", f"Received reply and making move {next_moves[0]}."
            )
